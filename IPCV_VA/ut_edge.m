...
if strcmp(threshold_method,'hysteresis')
    his=imhist(Iout, 100000);
    thres_parm(1) = min(find(cumsum(his) > (1-thres_parm(1))*prod(size(Iout)))) / 100000;
    thres_parm(2) = min(find(cumsum(his) > (1-thres_parm(2))*prod(size(Iout)))) / 100000;
    Iout_low = Iout>thres_parm(1);
    Iout_high = Iout>thres_parm(2);
    Iout=imreconstruct(Iout_high,Iout_low);
end
