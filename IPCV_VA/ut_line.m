function [lam1,dir, lam2]=ut_line(varargin)
%ut_line: find line strength and directions using the eigenvalues of the Hessian
%
% [LINE,DIR,LAMBDA]=UT_LINE(I,SIGMA1,SIGMA2) finds the line strength
...
% LAMBDA:   largest(lambda of Hessian).
%
...
% sort eigenvalues
i = find(abs(lam2)>abs(lam1));
lam1(i) = lam2(i);

% find eigenvector of largest eigenvalue
v = ones(length(lam1(:)),2);
i = find(abs(b)<100*eps);
j = find(abs(b)>=100*eps & abs(lam1)>0);
v(j,2) = (lam1(j) - a(j))./b(j);
v(i,2) = b(i)./(lam1(i)-c(i));
